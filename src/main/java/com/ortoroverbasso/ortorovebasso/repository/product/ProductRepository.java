...
public interface ProductRepository extends JpaRepository<ProductEntity, Long>, JpaSpecificationExecutor<ProductEntity> {
    ...
    @Query("""
                    SELECT DISTINCT p FROM ProductEntity p LEFT JOIN FETCH p.priceLargeQuantities WHERE p.id IN :ids
            """)
    List<ProductEntity> findAllWithPriceLargeQuantitiesByIds(@Param("ids") List<Long> ids);

    @Query("""
                    SELECT p FROM ProductEntity p LEFT JOIN FETCH p.priceLargeQuantities WHERE p.id IN :ids
            """)
    List<ProductEntity> findAllByIdWithPrices(@Param("ids") List<Long> ids);
}
